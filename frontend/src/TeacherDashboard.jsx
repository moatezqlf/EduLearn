...
const navItems = [
  ...
  { id: "history",    label: "Historique Session", icon: "◉" },
  { id: "communication", label: "Communication", icon: "💬", route: "/teacher/communication" },
...
export default function TeacherDashboard() {
  ...
  const statCards = [
    { label: "Total Students",   value: analytics ? String(analytics.students)   : "—", sub: "enrolled",       icon: "⊕", accent: "#6C63FF" },
    { label: "My Courses",       value: String(courses.length),                          sub: "total",          icon: "◫", accent: "#0EA5E9" },
    { label: "Pending Reviews",  value: String(pending),                                 sub: "need attention", icon: "✎", accent: "#EF4444", urgent: pending > 0 },
    { label: "Total Submissions",value: analytics ? String(analytics.submissions) : "—", sub: "all time",      icon: "◎", accent: "#10B981" },
  ];
      ...
      <aside style={{ width: sidebarOpen ? 220 : 64, transition: "width .25s ease", background: "#fff", borderRight: "1px solid #EAECF0", display: "flex", flexDirection: "column", padding: "20px 12px", flexShrink: 0, overflow: "hidden" }}>
        ...
        {sidebarOpen && (
          <div style={{ margin: "0 6px 14px", padding: "6px 10px", background: "#F0FDF4", borderRadius: 8, fontSize: 11, color: "#166534", fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#10B981", display: "inline-block" }}/> Teacher Account
          </div>
        )}
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
          {navItems.map(item => (
            <div key={item.id} className={`nav-item ${activeNav === item.id ? "active" : ""}`} onClick={() => item.route ? navigate(item.route) : setActiveNav(item.id)}>
        ...
        <div style={{ borderTop: "1px solid #F3F4F6", paddingTop: 14, display: "flex", alignItems: "center", gap: 10, padding: "14px 6px 0" }}>
          ...
          {sidebarOpen && (
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: "#1A1D23", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user?.name || "Teacher"}</div>
              ...
              {showDropdown && (
                <div className="dropdown-menu">
                  <div className="dropdown-item" onClick={() => { setShowDropdown(false); navigate("/teacher/courses/new"); }}>
                    ...
                    <div>
                      <p style={{ fontWeight: 600, fontSize: 13 }}>New Course</p>
